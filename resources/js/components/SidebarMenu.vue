<template>
    <div class="sidebar-layout">
        <div class="fixed w-webkit z-30 flex bg-white dark:bg-[#0F172A] p-2 items-center justify-center h-16 px-10">
            <div ref="logo" :class="logoClasses"
        ...
        <aside ref="sidebar"
            ...
            <!-- open sidebar button -->
            <div ref="maxToolbar" :class="maxToolbarClasses"
                ...
                <div class="flex pl-4 items-center space-x-2 ">
                    <div>
                        <div ref="moon" @click="setDark('dark')"
                        ...
                        <div ref="sun" @click="setDark('light')"
            ...
            <!-- MAX SIDEBAR-->
            <div ref="maxSidebar" :class="maxSidebarClasses"
            ...
            <!-- MINI SIDEBAR-->
            <div ref="miniSidebar" :class="miniSidebarClasses"
                class="mini mt-20 flex-col space-y-2 w-full h-[calc(100vh)]">
                <div
        ...
        <div ref="content" :class="contentClasses"
            class="content transform ease-in-out duration-500 pt-20 px-2 md:px-10 pb-4">
            <slot />
        </div>
    </div>
</template>
...
// Reactive state
const isMaxSidebar = ref(false);
const currentRoute = ref("");
const listRoute = ref([
    { name: "UnoCssPage", page: "UnoCss Page" },
    { name: "BlogForm", page: "Blog Form" },
...
const sidebarClasses = computed(() => [
    'fixed', 'left-0', 'top-0', 'bottom-0', 'bg-gray-800', 'transition-transform', 'duration-300',
    isMaxSidebar.value ? 'translate-x-none' : '-translate-x-48'
]);

const maxSidebarClasses = computed(() => [
    'transition-all', 'duration-300',
    isMaxSidebar.value ? 'flex' : 'hidden'
]);

const miniSidebarClasses = computed(() => [
    'transition-all', 'duration-300',
    isMaxSidebar.value ? 'hidden' : 'flex'
]);

const maxToolbarClasses = computed(() => [
    'transition-transform', 'duration-300',
    isMaxSidebar.value ? 'translate-x-0' : 'translate-x-24', isMaxSidebar.value ? '' : 'scale-x-0'
]);

const logoClasses = computed(() => [
    isMaxSidebar.value ? '' : 'ml-12'
]);

const contentClasses = computed(() => [
    'transition-all', 'duration-300',
    isMaxSidebar.value ? 'ml-12 md:ml-60' : 'ml-12'
]);

const setDark = (val) => {
    if (val === "dark") {
        document.documentElement.classList.add('dark');
        moon.value.classList.add("hidden");
        sun.value.classList.remove("hidden");
    } else {
        document.documentElement.classList.remove('dark');
        sun.value.classList.add("hidden");
        moon.value.classList.remove("hidden");
    }
};

const openNav = () => {
    isMaxSidebar.value = !isMaxSidebar.value;
};
</script>
